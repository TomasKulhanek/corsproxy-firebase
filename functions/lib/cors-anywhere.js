...
// proxyRequest modified to proxy PUT and POST requests with body
const { finished } = require('stream');

function proxyRequest(req, res, proxy) {
  const location = req.corsAnywhereRequestState.location;
  req.url = location.path;
  console.log(`[proxyRequest] Incoming ${req.method} ${location.href}`);
  ...
  const upstream = req.corsAnywhereRequestState.getProxyForUrl(location.href);
  if (upstream) {
    console.log(`[proxyRequest] Using upstream proxy: ${upstream}`);
    proxyOptions.target = upstream;
    proxyOptions.toProxy = true;
    req.url = location.href;               // full URL when we go through an upstream proxy
  }

  /* --------------------------------------------------- read the body once -- */
  let bodyBuffer = null;

  // 1.  If Firebase has already buffered the request, use it
  if (req.rawBody && req.rawBody.length) {
    bodyBuffer = Buffer.from(req.rawBody);
    //console.log(`[proxyRequest] Using req.rawBody (${bodyBuffer.length} bytes)`);
  } else {
    // 2.  Otherwise collect the chunks yourself
    const chunks = [];
    req.on('data', chunk => {
      //console.log(`[proxyRequest] received chunk (${chunk.length})`);
      chunks.push(chunk);
    });

    // finished() fires even if the stream had ended before this point
    finished(req, err => {
      if (err) {
        //console.error(`[proxyRequest] request stream error: ${err.stack || err}`);
        return proxy.emit('error', err, req, res);
      }
      bodyBuffer = Buffer.concat(chunks);
      //console.log(`[proxyRequest] total body: ${bodyBuffer.length} bytes`);
      startProxy();                        // we can proxy now
    });

    return;                                // wait for finished() callback
  }

  // rawBody path can jump straight to proxy:
  startProxy();

  /* ----------------------------------------------------------- helper ---- */
  function startProxy() {
    if (bodyBuffer) {
      proxyOptions.headers['content-length'] = bodyBuffer.length;
      proxy.once('proxyReq', proxyReq => {
        //console.log(`[proxyRequest] proxyReq → writing ${bodyBuffer.length} bytes`);
        proxyReq.end(bodyBuffer);          // write + finish
      });
    }

    proxy.once('proxyRes', proxyRes => {
      //console.log(`[proxyRequest] ← ${proxyRes.statusCode} from target`);
    });

    proxy.once('error', err => {
      console.error(`[proxyRequest] proxy error: ${err.stack || err}`);
    });

    //console.log(`[proxyRequest] Calling proxy.web()`);
...
exports.createServer = function createServer(options) {
  ...
  //re-add the proxy.on('proxyRes') handler to manually call withCORS() and inject headers into the proxied response.
